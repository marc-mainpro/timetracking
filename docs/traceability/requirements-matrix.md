...
| RF-USR-006 Recuperacion de contrasena (UI) | T60-07 | `ForgotPasswordComponent` (`/auth/recuperar-contrasena`), `ResetPasswordComponent` (`/restablecer-contrasena`), `AuthService.requestPasswordReset/resetPassword`, enlace desde `LoginComponent` | los dos anteriores | `forgot-password.component.spec`, `reset-password.component.spec`, `error-messages.service.spec`, `recuperacion-contrasena.spec.ts` (E2E: correo real en mailpit, token de un solo uso) |
...
| RF-NOT-002 Notificaciones por correo | T110-05, T170-02, T170-09 | `NotificationSender`, `NotificationEmailComposer` (saludo, enlace y pie), puerto `EmailSender`, columna `email_required` | — (tarea programada) | `NotificationSenderTest`, `NotificationEmailComposerTest` |
| RF-NOT-003 Eventos notificables | T110-04, T170-03..T170-08 | `NotificationEventListener` (fan-out por rol), `NotificationRecipients`, `RoleRecipientQuery` / `IdentityRoleRecipientQuery`, `AbsenceIntegrationEventMapper`, `ShiftIntegrationEventMapper`, `NotifyStuckQueues` | — | `NotificationEventListenerTest`, `IdentityRoleRecipientQueryIntegrationTest`, `ShiftIntegrationEventMapperTest`, `AbsenceIntegrationEventMapperTest`, `NotifyStuckQueuesTest` |
...
| RF-NOT-006 Trazabilidad del envío | T110-02, T170-02 | `Notification` (estado, intentos, último error, fechas, canal y ruta), migraciones V24 y V26 | — | `NotificationTest` |
| RF-NOT-007 Avisos por rol (administrador de tenant y de plataforma) | T170-01, T170-03, T170-06, T170-07 | `RoleRecipientQuery`, `NotificationRecipients.TenantRole` / `PlatformAdmins`, `UserRepository.findActiveByRole`, ADR-0018 | `GET /api/v1/notifications` | `NotificationEventListenerTest`, `IdentityRoleRecipientQueryIntegrationTest` |
| RF-NOT-008 Enlace accionable de la notificación | T170-02, T170-09, T170-10 | `Notification.actionPath`, `NotificationEmailComposer`, `NotificationsComponent.open` | `GET /api/v1/notifications` (`actionPath`) | `NotificationEmailComposerTest`, `notifications.component.spec` |
