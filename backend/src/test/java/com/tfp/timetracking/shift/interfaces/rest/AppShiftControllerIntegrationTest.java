...
class AppShiftControllerIntegrationTest {
    ...
    @Test
    void employeeCanReadOwnEffectiveShiftForDate() throws Exception {
        TestTenantFactory.TenantActors tenant = testTenantFactory.createTenantActors("shift-app");
        String templateResponse = mockMvc.perform(post("/api/v1/admin/shifts/templates")
        ...
        mockMvc.perform(get("/api/v1/app/shifts")
                        .param("date", "2026-09-15")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + tenant.employee().token()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name").value("Turno tarde"))
                .andExpect(jsonPath("$[0].crossesMidnight").value(false));
    }

    @Test
    void adminCannotUseEmployeeShiftEndpoint() throws Exception {
        TestTenantFactory.TenantActors tenant = testTenantFactory.createTenantActors("shift-app-role");
    ...
    @TestConfiguration
    static class AppShiftControllerIntegrationTestConfiguration {
